...
from std_msgs.msg import Bool, Float32  # Added Float32 here!
...
class SnapshotPublisher(Node):
    def __init__(self):
        ...
        # --- NEW: Subscribe to the dedicated confidence topic ---
        ...
        self.scan_msg = None
        self.amcl_msg = None
        self.conf_msg = None  # New state variable
    ...
    def conf_callback(self, msg): self.conf_msg = msg  # New callback
    ...
    def check_and_publish(self):
        # Updated guardrail to include the confidence message
        if not self.local_msg or not self.scan_msg or not self.amcl_msg or not self.conf_msg:
            self.get_logger().warn('Waiting for all topics (including /amcl_confidence) to arrive...')
            self.take_snapshot = False
            return
        ...
        # 1. Populate AMCL Guess & New Confidence Score
